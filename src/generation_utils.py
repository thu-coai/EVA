...
def generate_no_beam(model_batch, full_context, model, tokenizer: EVATokenizer, args, device):
    ...
    while gen_len < target_length:
        if unfinished_sents.max() == 0:
            ...
            tokens_to_add = next_token.to(unfinished_sents.device) * unfinished_sents + tokenizer.sep_id * (1 - unfinished_sents)
...
def generate_beam(model_batch, full_context, model, tokenizer: EVATokenizer, args, device):
    ...
    while gen_len < target_length:
        ...
        if do_sample:
            _scores = scores.to(beam_scores[0].device) + beam_scores[:, None].expand_as(scores.to(beam_scores[0].device))
