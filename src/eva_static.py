...
def generate_samples(model, contexts, tokenizer: EncDecTokenizer, args, device, ranker=None, ranker_tokenizer=None):
    init_list()
    ...
    with torch.no_grad():        
        ...
        for context in tqdm(contexts):
            ...
            if dist.get_rank() == 0:
                ...
                responses.append(tokenizer.decode(generation_token_ids))
...
def _count_ngram(preds, n):
    """
    Count the number of unique n-grams
    :param preds: list, a list of responses
    :param n: int, n-gram
    ...
    for resp in preds:
        ...
        for i in range(len(resp) - n + 1):
            unique_ngrams.add(' '.join(resp[i: i + n]))
            total_ngrams += 1
    return 1.0 * len(unique_ngrams) / total_ngrams


def eval_distinct(preds):
    """
    :param preds: list, a list of hyps responses
    :return: distinct score for 1, 2-gram
    """

    assert isinstance(preds[0], list)

    dist1 = _count_ngram(preds, 1)
    dist2 = _count_ngram(preds, 2)
...
def main():
    ...
    print('Model Loaded!')
    #generate samples
    test_path = ['dialog.jsonl', 'high-frequency-query.xlsx', 'random-query.xlsx', None][0]  # TODO
    if test_path == 'dialog.jsonl':
        contexts = []
        golds = []
        with open(os.path.join('test_data', test_path)) as fr:
            lines = fr.readlines()[:300]
            for line_idx, line in enumerate(lines):
                turns = json.loads(line.strip())
                for idx in range(1, len(turns)):
                    contexts.append(turns[:idx])
                    golds.append(turns[idx])

    elif test_path in ['high-frequency-query.xlsx', ]:
        df = pandas.read_excel(os.path.join('test_data', test_path), sheet_name='闲聊测试query筛选')
        print(df)
        contexts = [[post] for post in df['query']]
        golds = None
    elif test_path in ['random-query.xlsx', ]:
        df = pandas.read_excel(os.path.join('test_data', test_path), sheet_name='Sheet1')
        print(df)
        contexts = [[post] for post in df['query']]
        golds = None
    else:
        contexts = [["你好!"], ["很高兴见到你！"], ["你叫什么名字？"]]
        golds = ["你好呀！", "我也很高兴见到你！", "我叫EVA"]
    responses = generate_samples(model, contexts, tokenizer, args, torch.cuda.current_device(), ranker=ranker, ranker_tokenizer=ranker_tokenizer)
    if test_path == 'dialog.jsonl':
        with open('responses.json', 'w') as fw:
            json.dump(responses, fw)
    elif test_path in ['high-frequency-query.xlsx', 'random-query.xlsx']:
        df = dict(df)
        df['response'] = responses
        df = pandas.DataFrame(df)
        save_path = test_path[:-len('.xlsx')] + '-pred' + '.xlsx'
        df.to_excel(save_path)
    
    dist1, dist2 = eval_distinct([[w for w in response] for response in responses])

    print(responses)
    if golds:
        response_bleu_1_nltk = corpus_bleu(list_of_references=[[[w for w in ref]] for ref in golds],
                                           hypotheses=[[w for w in pred] for pred in responses],
                                           weights=(1.0, ),
                                           smoothing_function=SmoothingFunction().method1) * 100
        response_bleu_2_nltk = corpus_bleu(list_of_references=[[[w for w in ref]] for ref in golds],
                                           hypotheses=[[w for w in pred] for pred in responses],
                                           weights=(0.5, 0.5),
                                           smoothing_function=SmoothingFunction().method1) * 100
        response_bleu_3_nltk = corpus_bleu(list_of_references=[[[w for w in ref]] for ref in golds],
                                           hypotheses=[[w for w in pred] for pred in responses],
                                           weights=(1.0/3, 1.0/3, 1.0/3),
                                           smoothing_function=SmoothingFunction().method1) * 100
        print("response_bleu_1_nltk:", response_bleu_1_nltk)
        print("response_bleu_2_nltk:", response_bleu_2_nltk)
        print("response_bleu_3_nltk:", response_bleu_3_nltk)
